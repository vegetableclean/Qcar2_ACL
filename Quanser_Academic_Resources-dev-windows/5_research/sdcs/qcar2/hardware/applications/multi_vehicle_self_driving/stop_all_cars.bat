...
@REM Launching the script(s) on specific QCar

@REM Define the URL of the remote QUARC run-time manager
@REM Main QCar model

start "stop all cars" python python/stop.py -ip %QCAR_IPS%
for %%I in (%QCAR_IPS%) do (
    start "Stopping QUARC Models" quarc_run -q -Q -t tcpip://%%I:17000 *.rt-linux_qcar2
    timeout /t 1
    )
